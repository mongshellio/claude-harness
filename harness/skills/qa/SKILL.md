...
description: >-
  작업 마무리 직전 검증 단일 진입점.
  변경 도메인을 매칭한 뒤 toolchain 검증(typecheck/test/lint + DB schema 변경 시 schema 정합성 + decisions 파일 변경 시 인덱스 정합) / code-reviewer / doc-reviewer / harness-reviewer 를 기본 실행.
  build / security-reviewer 는 --branch(풀 검증) 또는 개별 opt-in(--build/--security) 시에만 실행. 외부 LLM 리뷰(Gemini CLI / DeepSeek API)는 --gemini / --deepseek opt-in 시에만 실행 (--branch 자동 포함 아님).
  browser 검증은 --branch + UI 변경 있을 때만 수행 (Preview MCP 기반, opt-in 없음).
  통일 분류 (공통 분류 등급) 로 리포트. 트리거 — "qa", "검증", "코드 리뷰".
---
...
| RANGE | 해당 라운드의 좁은 diff | 누적 변경 (기본 / `--branch` / `--staged`) |
| 검증 수행 | 검증 명령(bash) + 라우팅 매칭 reviewer (`security-reviewer` 제외) | `--branch` (풀): 검증 명령(bash) + `code-reviewer` + `doc-reviewer` + `harness-reviewer` + `security-reviewer` + browser(UI 변경 시); 기본/단발: 검증 명령(bash) + `code-reviewer` + opt-in 만; 외부 LLM(`gemini`/`deepseek`)은 `--gemini`/`--deepseek` 명시 시에만 |
...
`security-reviewer` = `/qa` 단독 호출 대상 (`/plan` 루프 안 자동 호출 X). 단발 기본에서는 `--security` opt-in 또는 `--branch` 시에만 실행.

`/plan` 사이클 종료 직후 `/qa` 호출 시 `code-reviewer` 가 중복 호출되는 인상이 있을 수 있다. 이는 **사이클 누적 시각**으로 보는 다른 input — 라운드별로는 잡히지 않는 cross-round 결함 검토 의의가 있다. 부담되면 `--skip-code-review` 옵션으로 `code-reviewer` 만 빼고 호출 가능 (doc-reviewer / harness-reviewer 는 그대로 실행; 외부 LLM 리뷰는 `--gemini`/`--deepseek` 명시 시에만).
...
> ⚠️ untracked 파일은 검증 대상 아님 (git diff 가 보지 못함). 신규 파일은 `git add` 후 `--staged` 또는 commit 후 호출.
...
- **코드 파일 변경 여부**: `git diff $RANGE --name-only | grep -v '\.md$'` 로 `.md` 외 변경 확인. toolchain 외 파일 (이미지/lock/data 등) false positive 는 도메인 외 입력 정책 (`.claude/README.md` "Reviewer 라우팅") 으로 흡수.
- **UI 변경 여부**:
...
> **라우팅 권위**: 변경 파일 → reviewer 분기는 `.claude/README.md` 의 "Reviewer 라우팅" 표를 따른다. 이 SKILL 은 caller — 라우팅 표를 자체 정의하지 않는다.
...
```bash
# RANGE: Step 0 결정값 (HEAD / main...HEAD / --cached)
...
# node .claude/scripts/check-decisions-index.mjs > "$QA_TMP/decisions-index.txt" 2>&1 &
...
**doc-reviewer Agent 호출 (doc-reviewer 도메인 — `**/*.md` 중 `.claude/**` 외 — 매칭 시):**

Agent 도구로 `subagent_type=doc-reviewer` 를 호출한다. 프롬프트에 다음을 포함:
- `"변경 범위: $RANGE. 호출 출처: /qa 스킬에서 doc-reviewer 도메인 매칭 시 자동 호출."`
- 변경 .md 파일 목록: `$QA_TMP/doc-domain.txt` 내용을 그대로 포함 (`.claude/**` 외 .md 만)

도메인 컨텍스트는 doc-reviewer 가 자체 구성한다 (적재 방식은 doc-reviewer 권위). 출력의 공통 분류 등급 레이블을 그대로 활용한다.

**harness-reviewer Agent 호출 (harness-reviewer 도메인 — `.claude/**/*.md` — 매칭 시):**

Agent 도구로 `subagent_type=harness-reviewer` 를 호출한다. 프롬프트에 다음을 포함:
- `"변경 범위: $RANGE. 호출 출처: /qa 스킬에서 harness-reviewer 도메인 매칭 시 자동 호출."`
- 변경 .md 파일 목록: `$QA_TMP/harness-domain.txt` 내용을 그대로 포함 (`.claude/**/*.md` 만)

하네스 컨텍스트는 harness-reviewer 가 tiered loading(Tier 0 인덱스 전체 + 조건부 Tier 2 후보 본문)으로 자체 구성한다 (적재 방식은 harness-reviewer 권위). 출력의 공통 분류 등급 레이블을 그대로 활용한다.

**Step 3c: browser 검증 (`--branch` + UI 변경 조건일 때만)**

Step 1 의 UI 변경 판정에서 UI 변경이 확인됐고 `--branch` 인 경우에만 실행한다.

메인 세션이 `preview_*` 도구를 순차 호출한다. 구체 진입 절차·라우트·점검 항목은 `.claude/skills/qa/browser-scenarios.md` 참조로 위임 (본문에 프로젝트 종속값 미기재).
...
typecheck / test 는 실패해도 stop 하지 않는다. P0 로 분류 후 리포트에 포함한다.
...
**code-reviewer**: ✅ 실행 / ⏭️ `--skip-code-review` 사용자 명시 스킵
**security-reviewer**: ✅ 실행 (`--branch` 또는 `--security`) / ⏭️ 단발 기본 — 스킵 (`--security` 또는 `--branch` 로 활성) / ⏭️ 코드 변경 없음 — 스킵
**Gemini CLI**: ✅ 사용 (`--gemini`) / ⏭️ 단발/풀 공통 — 스킵 (`--gemini` 명시 시에만 활성) / ⏭️ 사용자 스킵 선택 (미설치)
**DeepSeek API**: ✅ 사용 (`--deepseek`) / ⏭️ 단발/풀 공통 — 스킵 (`--deepseek` 명시 시에만 활성) / ⏭️ 사용자 스킵 선택 (키 부재)
**browser 검증**: ✅ 실행 (`--branch` + UI 변경 있음) / ⏭️ 단발 기본 — 스킵 / ⏭️ UI 변경 없음 — 스킵 / ⏭️ preview_start 실패 — 스킵
**doc-reviewer**: ✅ 실행 (.md 변경 있음) / ⏭️ .md 변경 없음 — 스킵
**harness-reviewer**: ✅ 실행 (.claude/** 변경 있음) / ⏭️ .claude/** 변경 없음 — 스킵
...
- `doc-reviewer`: ✅ 완료 / ⏭️ .md 변경 없음 — 스킵
- `harness-reviewer`: ✅ 완료 / ⏭️ .claude/** 변경 없음 — 스킵
```
...
- 기본(uncommitted) / `--staged` 호출은 부분 검증이라 PR 범위를 보증하지 못하므로 marker 를 기록하지 않는다.
...
수정 diff 가 닿은 도메인의 reviewer 만 재실행한다. 닿지 않은 도메인의 직전 라운드 결과는 그대로 승계한다.

| 수정 diff 도메인 | 재실행 대상 |
|----------------|-----------|
| 코드 파일 (.ts/.tsx 등) | code-reviewer (단, `--skip-code-review` 미지정 시) |
| `.claude/**/*.md` | harness-reviewer |
| `docs/**/*.md` (.claude 외) | doc-reviewer |
| 코드 파일 + 실질 delta (새 로직·새 파일·의미 변경) | security-reviewer (--branch/--security 조건 충족 시) |
...
| 상황 | 동작 |
|------|------|
| node_modules 부재 (필요 검증 호출 예정 시) | 자동 의존성 설치 후 진행. 설치 실패 시 stop + 안내 |
| 변경 없음 | 사용자 확인 후 진행 결정. 사용자가 "취소" 응답 시 스킬 종료 |
| gemini CLI 미설치 (gemini 실행 예정 시) | 안내 + 사용자 확인 ([재호출 → stop] / [스킵 이번 호출 → 나머지 계속]). 리포트 헤더에 선택 사유 명시 |
| `DEEPSEEK_API_KEY` 부재 또는 jq/curl 미설치 (deepseek 실행 예정 시) | 안내 + 사용자 확인 ([재호출 → stop] / [스킵 이번 호출 → 나머지 계속]). 리포트 헤더에 선택 사유 명시 |
| typecheck / test / lint 실패 | stop X. P0 분류 후 리포트 출력 |
| schema 정합성 실패 (`<migration-dir>/` 또는 ERD 산출물 dirty) | stop X. P0 분류 후 리포트 출력 |
| decisions 인덱스 정합 불일치 (`check-decisions-index.mjs` exit 1) | stop X. P0 분류 후 리포트 출력 |
| build 실패 (`--branch` 또는 `--build` 시) | stop X. P0 분류 후 리포트 출력 |
| Gemini API 오류 / 타임아웃 (`gtimeout -k 30 600` 초과 — quota hang 포함, SIGKILL 강제 종료) / 파싱 결과 없음 (gemini 실행 시) | warn + 리포트에 "Gemini 응답 실패" (`$QA_TMP/gemini-result.txt` 비어있으면 파싱 실패로 동일 처리) |
| DeepSeek API 오류 / 타임아웃 (`curl --max-time 600` 초과) / 응답 없음 (deepseek 실행 시) | warn + 리포트에 "DeepSeek 응답 실패" (`$QA_TMP/deepseek-result.txt` 비어있으면 동일 처리; `deepseek-err.txt` 에 HTTP/네트워크 오류 단서) |
| code-reviewer 호출 실패 | warn + 나머지 결과만 출력 |
| security-reviewer 호출 실패 (`--branch` 또는 `--security` 시) | warn + 나머지 결과만 출력 |
| .md 변경 없음 | doc-reviewer 스킵 (리포트에 ⏭️ 표시) |
| doc-reviewer 호출 실패 | warn + 나머지 결과만 출력 |
| `.claude/**/*.md` 변경 없음 | harness-reviewer 스킵 (리포트에 ⏭️ 표시) |
| harness-reviewer 호출 실패 | warn + 나머지 결과만 출력 |
| `--branch` + UI 변경 없음 | browser 검증 스킵 (리포트에 ⏭️ 표시) |
| `preview_start` 실패 (포트 점유 등) | 실행 중 인스턴스 재사용 시도 → 불가 시 browser 검증 스킵 (리포트 헤더에 사유 명시) |
| 알 수 없는 인자 | 사용법 안내 후 stop |
